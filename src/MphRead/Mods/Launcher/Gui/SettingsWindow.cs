...
namespace MphRead.Mods.Launcher.Gui
{
    ...
    /// inside a match, which is why nothing here needs a restart to take effect
    /// except the window mode.
    ///
    /// This is the whole settings window on every platform. It used to be
    /// WinForms and therefore Windows only, which left the other platforms with
    /// a card offering five choices and a note saying the rest was in the
    /// console menu -- that is, telling a person who opened a launcher to go
    /// and type instead.
    /// </summary>
    internal sealed class SettingsWindow : Window
    {
        ...
        public SettingsWindow(MenuSettings settings, bool inGame = false)
        {
            ...
            Grid.SetRow(railScroll, 0);
            Grid.SetRow(footer, 1);
            rail.Children.Add(railScroll);
            rail.Children.Add(footer);
            var railPanel = new Border
            {
                Background = GuiTheme.PanelBrush,
                Padding = new Thickness(18, 20, 14, 14),
                Width = _railWidth,
                Child = rail
            };
            ...
            grid.Children.Add(railPanel);
            grid.Children.Add(_pages);
            Content = grid;
        ...
        /// <summary>
        /// Put the keyboard on the first section as the window appears.
        ///
