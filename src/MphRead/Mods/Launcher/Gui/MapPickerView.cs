...
namespace MphRead.Mods.Launcher.Gui
{
    ...
    internal sealed class MapPickerView : UserControl
    {
        ...
        public MapPickerView(IReadOnlyList<string> rooms, string current, bool inGame = false)
        {
            // Over a match the backdrop would be a photo covering the game it
            // belongs to; away from one it is the same picture the main menu
            // uses.
            Background = inGame ? GuiTheme.InkBrush : Brushes.Transparent;
            Focusable = true;
            ...
            if (inGame)
            {
                // Its own window there, so a title bar across the top is the
                // window's own chrome and not a leftover of the old UI.
                ...
                var inGameBody = new ScrollViewer
                {
                    Content = grid,
                ...
                dock.Children.Add(inGameBody);
                Content = dock;
                return;
            }

            // The launcher path: the same corner Settings' own Back sits in,
            // no bar across the top.
            var flatBack = new MenuEntry("Back", titleSize: 20)
            {
                HorizontalAlignment = HorizontalAlignment.Left,
                VerticalAlignment = VerticalAlignment.Bottom,
                Margin = new Thickness(64, 0, 0, 96)
            };
            flatBack.Click += (_, _) => Closed?.Invoke(this, EventArgs.Empty);
